...
def train_step(batch_data, run_info):
    ...
    pred_dict["hv"] = pred_dict["np"][:, :, :, :2]
    pred_dict["np"] = F.softmax(pred_dict["np"][:, :, :, 2:], dim=-1)
    if model.module.nr_types is not None:
    ...
    loss_items_dict = {
        k: v.detach().cpu().numpy() for k, v in loss_items_dict.items()
    }
...
def valid_step(batch_data, run_info):
    ...
    with torch.no_grad():  # dont compute gradient
        ...
        pred_dict["hv"] = pred_dict["np"][:, :, :, :2]
        pred_dict["np"] = F.softmax(pred_dict["np"][:, :, :, 2:], dim=-1)[..., 1]
        if model.module.nr_types is not None:
...
def infer_step(batch_data, model):
    ...
    with torch.no_grad():  # dont compute gradient
        ...
        pred_dict["hv"] = pred_dict["np"][:, :, :, :2]
        pred_dict["np"] = pred_dict["np"][:, :, :, 2:]

        # pred_dict["np"] = F.softmax(pred_dict["np"], dim=-1)[..., 1:]
        pred_dict["np"] = crop_op(pred_dict["np"], [92, 92], data_format="NHWC")
        pred_dict["hv"] =  crop_op(pred_dict["hv"], [92, 92], data_format="NHWC")
        pred_dict["np"] = F.softmax(pred_dict["np"], dim=-1)[..., 1:]
        ...
        if "tp" in pred_dict:
            ...
            type_map = type_map.type(torch.float32)
            pred_dict["tp"] = type_map
            pred_dict["tp"] = crop_op(pred_dict["tp"], [92, 92], data_format="NHWC")
