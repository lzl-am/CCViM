...
from CPM17.config import Config
...
class TrainManager(Config):
    ...
    def run_once(self, opt, run_engine_opt, log_dir, prev_log_dir=None, fold_idx=0):
        ...
        ####
        loader_dict = {}  # 加载数据集的dict
        for runner_name, runner_opt in run_engine_opt.items():
            loader_dict[runner_name] = self._get_datagen(
                opt["batch_size"][runner_name], # train_batch_size, val_batch_size
                runner_name, # train or valid
                opt["target_info"]["gen"], #
        ...
        for net_name, net_info in net_info_opt.items():
            ...
            params=([p for p in net_desc.parameters()])
            optimizer = optimizer(params, **optimizer_args)
        ...
        for runner_name, runner in runner_dict.items():  # 加载各种回调函数
            ...
            for event, callback_list, in callback_info.items():
                for callback in callback_list:
                    if callback.engine_trigger:
                        ...
                        callback.triggered_engine = runner_dict[triggered_runner_name] # TriggerEngine("valid")
        ...
        # start the run loop
        main_runner.run(opt["nr_epochs"])  # 开始训练
    ...
    def run(self):
        ...
        prev_save_path = None
        for phase_idx, phase_info in enumerate(phase_list):  # 两个phase，前一个phase训练decoder，后一个phase训练所有层
            if len(phase_list) == 1:
